"""Polish-specific prosody transformations for TTS post-processing.

Polish phonetics differ from English in three key ways:
    1. Fixed penultimate stress (accent on second-to-last syllable)
    2. Syllable-timed rhythm (equal syllable durations vs English stress-timing)
    3. Question intonation on penultimate syllable (not sentence-final rise)

All functions are numpy-only and independently callable.
"""
...
logger = logging.getLogger("audiosmith.polish_prosody")

# Polish vowels including nasal vowels (ą, ę) and ó
POLISH_VOWELS = set("aeiouyóąę")


def _find_vowel_clusters(word: str) -> List[Tuple[int, int]]:
    """Find start/end indices of vowel clusters (syllable nuclei) in a word."""
    clusters: List[Tuple[int, int]] = []
    i = 0
    lower = word.lower()
    while i < len(lower):
        if lower[i] in POLISH_VOWELS:
            start = i
            while i < len(lower) and lower[i] in POLISH_VOWELS:
                i += 1
...
def apply_penultimate_stress(
    audio: np.ndarray, sr: int, text: str, intensity: float,
) -> np.ndarray:
    """Boost amplitude on the penultimate syllable of each word.

    Polish has fixed penultimate stress — the second-to-last syllable
    of every word receives primary stress. TTS engines trained primarily
    on English often misplace stress in Polish words.

    Args:
        audio: Mono float32 numpy array.
        sr: Sample rate in Hz.
        text: Polish text to analyze for stress positions.
        intensity: Processing strength (0.0-1.0).

    Returns:
        Audio with penultimate syllables emphasized.
    """
    ...
    for start, end, word in boundaries:
        clusters = _find_vowel_clusters(word)
        if len(clusters) < 2:
...
def apply_question_intonation(
    ...
    """Apply Polish question intonation — pitch rise on penultimate syllable.

    Unlike English (sentence-final rise), Polish questions rise on the
    penultimate syllable of the last word. This applies a subtle
    frequency shift to the last 30% of the audio.
...
def normalize_syllable_timing(
    ...
    """Compress per-word energy variance toward syllable-timed rhythm.

    Polish is syllable-timed (each syllable roughly equal duration)
    while English is stress-timed. TTS engines trained on English
    produce exaggerated energy variation between syllables.
    ...
    Returns:
        Audio with reduced inter-word energy variance.
    """
    if intensity < 0.01 or len(audio) < sr // 10 or not text.strip():
        return audio
